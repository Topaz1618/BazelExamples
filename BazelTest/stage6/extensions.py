...
class DetectionTaskManager(MongoDBManager):
    ...
    def get_tasks(self, start, end, user, is_admin, is_personal=None):
        ...
        tasks = self.detect_task_collection.find(query).skip(end - 3).limit(end - start)
        # tasks = self.detect_task_collection.find(query).sort('timestamp', -1).limit(3)
        ...
        all_task_list.sort(key=lambda x: x['create_time'], reverse=True)
        print(all_task_list)
